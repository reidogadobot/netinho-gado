...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const nomorOwner = ["559887583208@s.whatsapp.net"]
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '559887583208@s.whatsapp.net'
                        /******ApiKey Input******/
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'criador':
                  client.sendMessage(from, {displayname: "Meu dono^-^", vcaurd: vcaurd}, MessageType.contact, { quoted: mek})
	...
	case 'hidetag':
              ...
              await client.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Onde est� o nome de usu�rio, irm�o? ', text, {quoted: mek})
                                                if (!isUser) return reply(mess.only.daftarB)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'nulis': 
				case 'tulis':
					if (args.length < 1) return reply('Eu te disse para escrever, mana?')
                                        if (!isUser) return reply(mess.only.daftarB)
					teks = body.slice(7)
					reply(mess.wait)
